...
df_array = baciq[['ch0', 'ch1', 'ch2', 'ch3', 'ch4', 'ch5',
                  'ch6', 'ch7', 'ch8', 'ch9', 'ch10']].values
...
# The columns automatically get named starting 0 so we can genralize the code
df_values = pd.DataFrame(df_array_round)
...
# Separate the proteins into the two categories:
# one peptide proteins, multiple peptide proteins
# Note: The current code discards one peptide proteins
# we can estimate the confidence interval using the beta distribution
df_grouped = df.groupby('Protein ID').count().reset_index().rename(
    columns={0: 'count'})
...
if flag == 0:
    ...
    temporary2 = pd.DataFrame(
        df_manycoinsdata.drop(
            ['Protein ID', 'count', 'index', int1], axis=1
...
"""
sm = pystan.StanModel(model_name='baciq',
                      model_code=code)
